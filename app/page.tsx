...
function FAQItem({ question, answer }: { question: string, answer: string }) {
  ...
  return (
    <div className="border border-white/5 bg-white/[0.02] rounded-2xl overflow-hidden transition-all duration-300 hover:border-yellow-500/30">
      <button 
        onClick={() => setIsOpen(!isOpen)} 
        className="w-full text-left px-6 py-5 flex items-center justify-between gap-4 focus:outline-none"
      >
        <div className="flex items-center gap-4">
          <CheckCircle className={`w-5 h-5 flex-shrink-0 transition-colors duration-300 ${isOpen ? 'text-yellow-400' : 'text-zinc-600'}`} />
          <h3 className="font-medium text-lg text-zinc-100">{question}</h3>
        </div>
        <ChevronDown className={`w-5 h-5 flex-shrink-0 transition-transform duration-300 ${isOpen ? 'rotate-180 text-yellow-400' : 'text-zinc-600'}`} />
      </button>
...
export default function HomePage() {
  ...
  const features = [
    { icon: FileText, title: 'Smart Invoicing', desc: 'Create professional invoices in seconds with auto-numbering, tax calculations, and multi-currency support.', accent: '#facc15' },
    ...
    { icon: Moon, title: 'Dark Mode', desc: 'Work comfortably day or night. FastWay Invoice adapts to your environment automatically.', accent: '#facc15' },
  ];
  ...
  return (
    <div className="min-h-screen text-white overflow-x-hidden" style={{ background: '#050505' }}>
      ...
      <div className="fixed top-[-200px] left-[-200px] w-[600px] h-[600px] rounded-full pointer-events-none" style={{ background: 'radial-gradient(circle, rgba(234,179,8,0.12) 0%, transparent 70%)' }} />
      ...
      {/* NAV */}
      <nav className="absolute top-0 inset-x-0 z-50 flex items-center justify-between px-8 py-6 max-w-7xl mx-auto w-full">
        <div className="flex items-center gap-2.5">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'rgba(234,179,8,0.1)' }}>
            <ReceiptText className="w-5 h-5" style={{ color: '#facc15' }} />
          </div>
          <span className="font-bold text-lg tracking-tight" style={{ fontFamily: 'var(--font-poppins)' }}>FastWay Invoice</span>
        </div>
        ...
        <Link href="/dashboard" className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 hover:scale-105 text-white" style={{ background: 'linear-gradient(135deg, #eab308, #ca8a04)', boxShadow: '0 0 30px rgba(202,138,4,0.4)' }}>
      ...
      <section id="features" className="relative z-10 max-w-7xl mx-auto px-8 py-24">
        ...
        >
          <p className="text-sm font-medium tracking-widest uppercase mb-4" style={{ color: '#facc15' }}>Everything you need</p>
          ...
          {steps.map(({ num, title, desc }, i) => (
            ...
            >
              <div className="relative z-10">
                <p className="text-5xl font-bold mb-4 leading-none" style={{ fontFamily: 'var(--font-poppins)', color: 'rgba(234,179,8,0.3)' }}>{num}</p>
                <h3 className="font-semibold text-white mb-2">{title}</h3>
                <p className="text-sm leading-relaxed" style={{ color: 'rgba(255,255,255,0.4)' }}>{desc}</p>
              </div>
            </motion.div>
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-8 py-16 mb-16">
        <motion.div
          ...
          style={{ background: 'linear-gradient(135deg, rgba(202,138,4,0.15) 0%, rgba(52,211,153,0.05) 100%)', borderColor: 'rgba(234,179,8,0.2)' }}
        >
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="w-96 h-96 rounded-full blur-3xl opacity-20" style={{ background: 'radial-gradient(circle, #eab308, transparent)' }} />
          </div>
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-medium mb-6 border" style={{ background: 'rgba(234,179,8,0.1)', borderColor: 'rgba(234,179,8,0.3)', color: '#facc15' }}>
            ...
            <Link href="/dashboard" className="group inline-flex items-center gap-3 px-10 py-5 rounded-2xl text-lg font-bold transition-all duration-300 hover:scale-105 text-white" style={{ background: 'linear-gradient(135deg, #eab308, #ca8a04)', boxShadow: '0 0 60px rgba(202,138,4,0.6)' }}>
              Start Invoicing Now
      ...
      <footer className="relative z-10 px-8 py-8 max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #eab308, #facc15)' }}>
            <ReceiptText className="w-4 h-4 text-white" />
          </div>
          <span className="font-bold text-sm" style={{ fontFamily: 'var(--font-poppins)', color: 'rgba(255,255,255,0.7)' }}>FastWay Invoice</span>
        </div>
        <p className="text-xs" style={{ color: 'rgba(255,255,255,0.2)' }}>© {new Date().getFullYear()} FastWay Invoice · Built for professionals</p>
